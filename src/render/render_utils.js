...
function draw_traffic_light(canvas_object) {
    ...
    let X = canvas_object.X;
    let Y = canvas_object.Y;
    let shift_center = 1.3;
    let shift_side = 1.3;
    //rotating
    context.rotate(canvas_object.angle);


    if (canvas_object.is_vertical) {
        context.drawImage(canvas_object,
            logic_to_canvas_multiplier * (-shift_center - shift_center - canvas_object.width ),
            logic_to_canvas_multiplier * (shift_center + 2 * shift_side  ),
            canvas_object.width * logic_to_canvas_multiplier,
            canvas_object.height * logic_to_canvas_multiplier);

        context.drawImage(canvas_object,
            logic_to_canvas_multiplier * (shift_center + shift_center ),
            logic_to_canvas_multiplier * (shift_center + 2 * shift_side  ),
            canvas_object.width * logic_to_canvas_multiplier,
            canvas_object.height * logic_to_canvas_multiplier);

        context.drawImage(canvas_object,
            logic_to_canvas_multiplier * (shift_center + shift_side ),
            logic_to_canvas_multiplier * (-shift_center - 2 * shift_side - canvas_object.height  ),
            canvas_object.width * logic_to_canvas_multiplier,
            canvas_object.height * logic_to_canvas_multiplier);
        ...
        context.drawImage(canvas_object,
            logic_to_canvas_multiplier * ( -shift_center - shift_side - canvas_object.width ),
            logic_to_canvas_multiplier * (shift_center + shift_center + shift_side ),
            canvas_object.width * logic_to_canvas_multiplier,
            canvas_object.height * logic_to_canvas_multiplier);
